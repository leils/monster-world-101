...
def initialize_actions(action_list, entityList): 
	for entity in entityList: 
		if isinstance(entity, entities.CSCStudent): 
			action_list.insert(entity, entity.rate)

def handle_ticks(grid, action_list, ticks):
   while not action_list.empty() and action_list.head().ord < ticks:
      ...
      if isinstance(action.item, entities.CSCStudent): 
         ...
         rescheduleItem(action_list, ticks, action.item)

def rescheduleItem(action_list, ticks, item): 
   action_list.insert(item, ticks + item.rate)